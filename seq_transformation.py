...
def split_dataset(skes_joints, label, performer, setup, evaluation, save_path):
    ...
    # Проверим индексы трейна
    print(f"Shape of skes_joints: {skes_joints.shape}")
    print(f"Max index in train_indices: {max(train_indices)}")
    # Ограничим train_indices в соотв. с skes_joints
    train_indices = [idx for idx in train_indices if idx < skes_joints.shape[0]]

    train_x = skes_joints[train_indices]
    train_y = one_hot_vector(train_labels)


    # Ограничим test_indices в соотв. с skes_joints
    test_indices = [idx for idx in test_indices if idx < skes_joints.shape[0]]
