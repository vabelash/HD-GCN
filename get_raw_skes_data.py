...
def get_raw_bodies_data(skes_path, ske_name, frames_drop_skes, frames_drop_logger):
    ...
    assert osp.exists(ske_file), f'Error: Skeleton file {ske_file} not found' 
    ...
    # Обработка данных с пропущенными кадрами
    ...
    if num_frames_drop < num_frames:
        print(f"Subject {ske_name} has insufficient frames")
...
if __name__ == '__main__':
    ...
    # Обработка существующих файлов, вывод в текстовый файл списка имен доступных файлов !!!
    # skes_name_file = osp.join(stat_path, 'skes_available_name.txt')
    skes_name_file = "C:\Projects\HD-GCN\\tmp_for_me"
